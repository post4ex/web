...
function _uploadActionBtns(url, uploadUid) {
    ...
    return [
        ...
        `<button onclick="(async()=>{try{const b=await fetchFileUrl('${url}');const a=document.createElement('a');a.href=b;a.download='${url.split('/').pop()}';a.click();}catch(e){showNotification('Download failed','error');}})()" title="Download" class="doc-action-btn">${_docIco.download}</button>`,
