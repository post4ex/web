...
const _docIco = {
    ...
    layout: `<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>`,
    upload: `<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5-5m0 0l5 5m-5-5v12"/></svg>`,
};
...
function renderDocumentCenter(order) {
    ...
    if (!window._labelLayout) window._labelLayout = '2up-landscape';
    const curLayout = window._labelLayout;
    const isPortrait = curLayout === '4up-portrait';

    const headerBtns = [
        `<button onclick="window.miniUploader?.setReference('${r}')" title="Upload" class="p-1.5 text-green-600 rounded hover:bg-green-50">${_docIco.upload}</button>`,
        `<button onclick="toggleLabelLayout()" id="layout-toggle-btn" title="${isPortrait ? '2×2 Portrait — tap for 1×2' : '1×2 Landscape — tap for 2×2'}" class="p-1.5 rounded ${isPortrait ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:bg-gray-100'}">${_docIco.layout}</button>`,
...
// --- LABEL LAYOUT TOGGLE ---
function toggleLabelLayout() {
    const newLayout = window._labelLayout === '4up-portrait' ? '2up-landscape' : '4up-portrait';
    setLabelLayout(newLayout);
}

function setLabelLayout(layout) {
    window._labelLayout = layout;
    const btn = document.getElementById('layout-toggle-btn');
    if (btn) {
        const isPortrait = layout === '4up-portrait';
        btn.classList.toggle('bg-blue-100', isPortrait);
        btn.classList.toggle('text-blue-700', isPortrait);
        btn.classList.toggle('text-gray-500', !isPortrait);
        btn.classList.toggle('hover:bg-gray-100', !isPortrait);
        btn.title = isPortrait ? '2×2 Portrait — tap for 1×2' : '1×2 Landscape — tap for 2×2';
    }
}
