    ...
    function setupAutocomplete(inputElement, resultsElement, displayElement, type) {
        ...
        function renderResults(filteredList) {
            ...
            if (filteredList.length === 0) {
                addNewLi.classList.add('active', 'bg-blue-100', 'text-blue-900');
            }
...
function renderLastBooked(ref) {
                    ...
                    <div class="bo-li-main">
                        ...
                        <div class="bo-li-btns">
                            <div class="bo-action-row">
                                ...
                                <button onclick="window.miniUploader?.setReference('${ref}')" title="Upload" style="padding:4px;border:none;background:transparent;cursor:pointer;color:#15803d;border-radius:4px;" onmouseover="this.style.background='#f0fdf4'" onmouseout="this.style.background='transparent'"><svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg></button>
                            </div>
    ...
    window.updateAndDisplayCharges = updateAndDisplayCharges;
    window.recalcTotalsFromCharges = (frightVal, otherCharges) => {
        ...
        set('display_other_chg', otherCharges);
        set('display_taxable', subtotal);
        set('display_sgst', sgst); set('display_cgst', cgst); set('display_igst', igst);
        set('display_gst_total', sgst + cgst + igst);
        set('display_total', total);
    };
    window.updateChargesFromFright = (frightVal) => {
        // Recalculate fright-dependent charges
        if (Object.keys(selectedCustomerDetails).length > 0) {
            const fuel = frightVal * (parseFloat(selectedCustomerDetails['FUEL_CHARGES']) || 0);
            const dev  = frightVal * (parseFloat(selectedCustomerDetails['DEV_CHARGES'])  || 0);
            const topayEl = document.getElementById('payment_topay');
            const topay = topayEl?.checked && selectedCustomerDetails['PCT_TOPAY_IF']
                ? Math.max(150, frightVal * parseFloat(selectedCustomerDetails['PCT_TOPAY_IF']))
                : parseFloat(document.getElementById('display_topay_chg')?.textContent) || 0;
            const set = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v.toFixed(2); };
            set('display_fuel_chg', fuel);
            set('display_dev_chg', dev);
            set('display_topay_chg', topay);
        }
        const otherChargeIds = ['display_fuel_chg','display_cod_chg','display_topay_chg','display_fov_chg','display_eway_chg','display_awb_chg','display_pack_chg','display_dev_chg'];
        const other = otherChargeIds.reduce((s, id) => s + (parseFloat(document.getElementById(id)?.textContent) || 0), 0);
        window.recalcTotalsFromCharges(frightVal, other);
    };
});
