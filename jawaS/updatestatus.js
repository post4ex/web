...
(function () {
    ...
    function injectModalHTML() {
        ...
        const modalHTML = `
        <div id="updateStatusModal" class="fixed inset-0 z-50 hidden bg-black/60 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
            <div class="bg-white rounded-xl shadow-2xl border border-gray-200 w-full max-w-md my-auto flex flex-col max-h-[92vh] sm:max-h-[90vh] overflow-hidden transform transition-all">
                <!-- Modal Header (Fixed top) -->
                <div class="flex justify-between items-center px-4 py-3 border-b border-gray-200 bg-gray-50 shrink-0">
                    <div class="flex items-center gap-2">
                        <h3 class="font-bold text-gray-800 text-sm sm:text-base">Update Shipment Status</h3>
                        <span class="text-xs font-mono text-gray-600 bg-gray-200/80 px-2 py-0.5 rounded" id="usm-ref-display">REF: ---</span>
                    </div>
                    <button type="button" onclick="UpdateStatusModal.close()" class="text-gray-400 hover:text-red-500 p-1.5 rounded-lg hover:bg-gray-100 transition-colors" title="Close">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                </div>

                <!-- Modal Body (Scrollable form contents) -->
                <form id="updateStatusForm" onsubmit="UpdateStatusModal.submit(event)" class="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 overscroll-contain">
                ...
                <div class="px-4 py-3 border-t border-gray-100 bg-gray-50 flex items-center justify-end gap-2 shrink-0">
                    <button type="button" onclick="UpdateStatusModal.close()" class="px-4 py-2 text-xs font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-200/60 rounded-md transition-colors">
                        Cancel
                    </button>
                    <button type="submit" form="updateStatusForm" id="usm-submit-btn" class="px-5 py-2 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 active:bg-blue-800 rounded-md shadow-sm transition-colors flex items-center gap-1.5">
